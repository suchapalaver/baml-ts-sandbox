...
async fn test_llm_tool_calling_rust() {
    ...
    baml_manager
        .register_tool(CalculatorToolTest)
        .await
        .unwrap();
    ...
    // Test that tools are registered and can be executed
    {
        ...
        let calc_result = manager
            .execute_tool("calculate", json!({"expression": "2 + 2"}))
            .await
            .unwrap();
...
async fn test_llm_tool_calling_js() {
    ...
    impl BamlTool for ReverseStringTool {
        ...
        ) -> baml_rt::error::Result<serde_json::Value> {
            let obj = args.as_object().expect("Expected object");
            let text = obj
                .get("text")
                .and_then(|v| v.as_str())
    ...
    assert!(
        tool_exists,
        "Tool 'reverse_string' should be registered in QuickJS"
    );
