...
async fn test_e2e_js_tool_with_llm() {
    ...
    // Verify JS tool is registered
    {
        ...
        assert!(
            js_tools.contains(&"format_text_js".to_string()),
    ...
    let llm_result = manager
        .invoke_function("SimpleGreeting", json!({"name": "JavaScript Tools"}))
        .await;
    ...
    match llm_result {
        Ok(greeting_value) => {
            ...
            assert_eq!(
                processing_obj
                    .get("originalGreeting")
                    .and_then(|v| v.as_str())
                    .unwrap(),
                &greeting_json,
                "Should contain original LLM greeting"
            );
...
async fn test_e2e_js_tool_workflow_llm_to_js() {
    ...
    let llm_result = manager
        .invoke_function("SimpleGreeting", json!({"name": "Text Analysis"}))
        .await;
    ...
    match llm_result {
        Ok(greeting) => {
            ...
            tracing::info!(
                "   Workflow: LLM → '{}' → JS Tool (analyze_text_js) → Analysis",
                text
            );
