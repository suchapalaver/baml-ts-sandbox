...
#[path = "../common.rs"]
mod common;
use common::{CalculatorTool, WeatherTool};
...
impl BamlTool for ArithmeticTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> baml_rt::error::Result<serde_json::Value> {
        ...
        let result = match op {
            ...
            _ => {
                return Err(baml_rt::error::BamlRtError::InvalidArgument(format!(
                    "Unknown operation: {}",
                    op
                )));
            }
...
async fn test_e2e_trait_tool_registration_rust_execution() {
    ...
    baml_manager
        .register_tool(StringManipulationTool)
        .await
        .unwrap();
    ...
    // Test 2: Execute string manipulation tool from Rust
    {
        ...
        tracing::info!(
            "✅ String manipulation tool executed successfully: {:?}",
            result
        );
    }
    ...
    // Test 3: List tools
    {
        ...
        assert!(
            tools.contains(&"arithmetic".to_string()),
            "Should list arithmetic tool"
        );
        assert!(
            tools.contains(&"string_manip".to_string()),
            "Should list string_manip tool"
        );
    ...
    // Test 4: Get tool metadata
    {
        ...
        tracing::info!(
            "✅ Tool metadata retrieval works: name={}, description={}",
            metadata.name,
...
async fn test_e2e_trait_tool_js_registration() {
    ...
    let check_result = bridge
        .evaluate(js_code_check)
        .await
        .expect("Should check tool registration");

    let check_obj = check_result.as_object().expect("Expected object");
    let arithmetic_exists = check_obj
        .get("arithmeticExists")
        .and_then(|v| v.as_bool())
        .unwrap_or(false);
    let string_manip_exists = check_obj
        .get("stringManipExists")
        .and_then(|v| v.as_bool())
        .unwrap_or(false);
    let tool_invoke_exists = check_obj
    ...
    assert!(
        arithmetic_exists,
        "Arithmetic tool should be registered in QuickJS"
    );
    assert!(
        string_manip_exists,
        "String manipulation tool should be registered in QuickJS"
    );
    assert!(
        tool_invoke_exists,
        "__tool_invoke helper should be registered in QuickJS"
    );
    ...
    // Verify we can still execute tools from Rust (the bridge works)
    {
        let manager = baml_manager.lock().await;
        let result = manager
            .execute_tool("arithmetic", json!({"operation": "add", "a": 15, "b": 27}))
            .await
            .unwrap();
...
async fn test_e2e_trait_tool_metadata_and_listing() {
    ...
    // Test getting metadata for each tool
    {
        ...
        let arithmetic_metadata = manager
            .get_tool_metadata("arithmetic")
            .await
            .expect("Should get arithmetic tool metadata");
        ...
        tracing::info!(
            "✅ Arithmetic tool metadata: name={}, description={}",
            arithmetic_metadata.name,
            arithmetic_metadata.description
        );

        // Test string manipulation tool metadata
        let string_manip_metadata = manager
            .get_tool_metadata("string_manip")
            .await
        ...
        tracing::info!(
            "✅ String manipulation tool metadata: name={}, description={}",
            string_manip_metadata.name,
            string_manip_metadata.description
        );
    }
...
async fn test_e2e_trait_tool_llm_calling() {
    ...
    // Test: LLM calls BAML function that returns a tool choice, we execute it
    {
        ...
        let tool_choice_result = manager
                ...
                match manager.execute_tool_from_baml_result(tool_choice).await {
                    Ok(tool_result) => {
                        ...
                        assert!(
                            ...
                            "Tool result should contain expected fields. Got: {:?}",
                            result_obj.keys()
                        );
                        ...
                        if let Some(result_val) = result_obj.get("result").and_then(|v| v.as_f64())
                        {
                            ...
                            tracing::info!(
                                "✅ LLM correctly chose calculator tool and got correct result: {}",
                                result_val
                            );
                        }
                    }
                    Err(e) => {
                        ...
                        tracing::info!(
                            "✅ LLM call completed successfully (tool execution issue is separate concern)"
                        );
