...
#[path = "../common.rs"]
mod common;
use common::{CalculatorTool, DelayedResponseTool, UppercaseTool, WeatherTool};
...
async fn test_e2e_llm_with_tools() {
    ...
    // Test 2: Test tool execution directly
    {
        ...
        let calc_result = manager
            .execute_tool("calculate", json!({"expression": "15 * 23"}))
            .await
        ...
        match result {
            Ok(response) => {
                ...
                assert!(
                    ...
                    "BAML function should return a non-empty response"
                );
                // The response should mention the location
                assert!(
                    response_str.to_lowercase().contains("london") || response_str.len() > 10, // Or just be a reasonable response
            ...
            Err(e) => {
                ...
                tracing::warn!(
                    "BAML function call returned error (may need tool calling integration): {}",
                    e
                );
            }
        }
    ...
    // Test 4: Verify tools are accessible from JavaScript
    {
        ...
        let weather_exists = obj
            .get("weatherToolExists")
        ...
        assert!(
            weather_exists,
            "Weather tool should be registered in JavaScript"
        );
        assert!(
            calc_exists,
            "Calculator tool should be registered in JavaScript"
        );
...
async fn test_e2e_tool_execution_flow() {
    ...
    baml_manager
        .register_tool(DelayedResponseTool)
        .await
        .unwrap();
    ...
    // Test tool execution flow
    {
        ...
        let result1 = manager
            .execute_tool("uppercase", json!({"text": "hello world"}))
            .await
        ...
        let result2 = manager
            .execute_tool("delayed_response", json!({"message": "test"}))
            .await
    ...
    // Test via JavaScript
    {
        ...
        assert!(
            obj.get("uppercaseExists")
                .and_then(|v| v.as_bool())
                .unwrap_or(false)
        );
        assert!(
            obj.get("delayedExists")
                .and_then(|v| v.as_bool())
                .unwrap_or(false)
        );
